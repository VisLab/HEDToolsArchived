...
tagstring|A search string consisting of tags to use for extracting data epochs. The string uses commas to narrow the search. Two tags separated by a comma is equivalent to the AND operator in general searches and the search only returns events that contain both of the tags. For example, “/Attribute/Visual/Color/Green, /Item/2d shape/Rectangle/Square” find epochs with green squares.
