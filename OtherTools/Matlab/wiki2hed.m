...
%
% Usage:
%   >>  xmlDoc = wiki2hed(wiki)
%
% Input:
%       'wiki'      The name or the path of the wiki text file containing
...
%       Optional:
%       'output'    The name or the path to the HED XML output file
...
%
% Output:
%       'xmlDoc'    Returns a handle to the newly created XML document
...
%
% Examples:
%                   To convert a wiki text file 'HED2.026.txt' containing
%                   all of the HED tags into a HED XML file 'HED2.026.xml'.
...
function xmlDoc = wiki2hed(wiki, varargin)
p = parseArguments();
attributeUnits = '';
hedLines = false;
nodeStruct = struct;
...
try
    ...
    function p = parseArguments()
        ...
        p = inputParser();
        p.addRequired('Wiki', @(x) ~isempty(x) && ischar(x));
        [path, file] = fileparts(wiki);
        p.addOptional('OutputFile', fullfile(path, [file '.xml']), ...
            @(x) ~isempty(x) && ischar(x));
        p.parse(wiki, varargin{:});
        p = p.Results;
    end % parseArguments
