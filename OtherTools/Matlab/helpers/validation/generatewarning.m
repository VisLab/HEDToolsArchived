...
switch(type)
    case 'cap'
        warning = sprintf(['\tEach slash-separated string in %s should' ...
            ' have the first letter captilized or be camel case\n'], tag);
    case 'line'
        warning = sprintf('Warnings on line %s:\n', num2str(line));
    case 'slash'
        warning = sprintf(['\t%s should start with a slash and should' ...
