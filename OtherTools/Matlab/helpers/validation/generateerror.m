...
switch(type)
    case 'comma'
        error = sprintf(['\t%s may contain a comma when no commas are' ...
            ' allowed in tags'], tag);
    case 'isNumeric'
        error = sprintf(['\t%s should have a number, and optionally a' ...
            ' unit as the leaf string\n'], tag);
    case 'line'
    ...
    case 'unique'
        error = sprintf(['\t%s is part of the unique tag set' , ...
            ...
            ' this tag group\n'], tag, prefix);
    case 'unitClass'
        error = sprintf(['\t%s should have one of %s as a unit, or', ...
            ' no unit\n'], tag, units);
end % generateerror