...
public class TestTagComments {
	...
	private static TagComments parentTagComment;
	...
	public static void classSetup() throws Exception {
		...
		parentTag.reset(parentTagUuid, parentTagPathname, null, null, 0,
				"test@email.com");
		parentTag.insertTag();
		parentTagComment = new TagComments(dbCon);
		Timestamp date = new Timestamp(System.currentTimeMillis());
		parentTagComment.reset(UUID.randomUUID(), parentTagUuid, date,
				"john doe", "this is a test comment 1");
		parentTagComment.insertTagComment();
		parentTagComment.reset(UUID.randomUUID(), parentTagUuid, date,
				"john doe", "this is a test comment 2");
		parentTagComment.insertTagComment();
	}
	...
	@Test
	public void testRetrieveCommentsByTagUuid() throws Exception {
		System.out
				.println("It should retrieve the tag attribute by the tag UUID");
		ArrayList<TagComments> retrievedTagComments = TagComments
				.retrieveCommentsByTagUuid(dbCon, parentTagUuid);
		assertEquals("There should be 2 comments for the retrieved tag",
				retrievedTagComments.size(), 2);
	}
